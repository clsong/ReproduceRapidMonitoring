#' Reproduce the data used in Figure Sxx in the supplement
#'
#' @export
plot_SI_figure_S <- function(nspecies = 20){
  plan(multisession, workers = 8)

  set.seed(1010)
  df <- expand_grid(
    nrow = floor(nspecies/2),
    ncol = floor(nspecies/2),
  ...
  df_plot %>%
    ...
    # directlabels::geom_dl(label=as.factor(df_plot$motif_type), method="last.bumpup", inherit.aes=T) +
    labs(
      y = "Proportion of subnetworks",
      x = "Distance to the centroid",
      title = paste0(nspecies, " species")
    ) +
    jtools::theme_nice() +
    theme(
  ...
  # ggsave('SI_phase_transition_70species.pdf', width = 6 / .618, height = 6)
}
